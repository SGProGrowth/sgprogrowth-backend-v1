...
function getPageMeta(pathname: string, role: UserRole) {
  ...
  if (role === 'instructor') {
    ...
    if (pathname.startsWith('/instructor/students/')) return { title: 'Student Progress', subtitle: 'Individual learner tracking' }
  }
...
export function DashboardLayout({ role }: { role: UserRole }) {
  ...
  return (
    <div className="flex min-h-screen bg-stone-100/80">
